...
class MysqlDumpManager extends BaseDumpManager
{
     ...
     * @return mixed
     */
    public function makeDumpCommand(
              $path,
        array $dbInfo,
        array $dumpOptions
    ) {
        ...
        $arguments = [
            dirname(dirname($_SERVER['DOCUMENT_ROOT'])).'\xampp\mysql\bin\mysqldump',
            '--host='.$dbInfo['host'],
            '--port='.$dbInfo['port'],
     ...
     * @return mixed
     */
    public function makeRestoreCommand(
              $path,
        array $dbInfo,
        ...
        $arguments = ArrayHelper::merge($arguments, [
            dirname(dirname($_SERVER['DOCUMENT_ROOT'])).'\xampp\mysql\bin\mysql',
            '--host='.$dbInfo['host'],
            '--port='.$dbInfo['port'],
            '--user='.$dbInfo['username'],
            '--password='.$dbInfo['password'],
